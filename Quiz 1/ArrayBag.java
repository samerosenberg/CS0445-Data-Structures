...
// equals bag[where]; otherwise, anEntry is not in the array
    ...
    /** Check to see if two bags are equal.
    ...
    public boolean equals(ArrayBag<T> aBag) {
        ...
        // This code is not the most efficient as it involves multiple
	...
	public boolean splitInto(BagInterface<T> first, BagInterface<T> second) {
        checkInitialization();
        double length = numberOfEntries;
        /*if(first.getCurrentSize()==0 || second.getCurrentSize()==0)
         return false;*/
   		for(int i=0;i<length;i++){
           if(first.add(bag[i]));
           else return false;
         }
         for(int i=0;i<numberOfEntries-length;i++){
            second.add(bag[i]);
         }
		return true;
	}
	...
	public boolean addAll(BagInterface<T> toAdd) {
		checkInitialization();
      boolean result = true;
      if(toAdd.getCurrentSize()==0)
         return false;
      for(int i=0;i<toAdd.getCurrentSize();i++){
      if (isArrayFull()) {
          result = false;
          break;
      } else { // Assertion: result is true here
          bag[numberOfEntries] = toAdd.remove();
          numberOfEntries++;
      }
   }// end if
      return result;
